...
test("first-run setup leads to a usable German overview", async ({ page }) => {
  ...
  await expect(page.getByLabel("Basiswährung")).toHaveValue("EUR")
  ...
  await expect(page.getByText("Maximal verfügbar")).toBeVisible()
  await expect(page.getByText("Geschützter Puffer").first()).toBeVisible()
})
