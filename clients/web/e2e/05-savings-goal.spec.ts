...
test("savings goal and contribution build progress in German", async ({ page }) => {
  ...
  await goalsCard.getByPlaceholder("Regelmäßiger Beitrag").fill("100")
  ...
  await pickOption(fundingCard.locator("select").filter({ hasText: "Sparzweck auswählen" }).first(), "Notgroschen E2E")
