...
test("income confirmation shows variance in German", async ({ page }) => {
  ...
  await incomeCard.getByRole("button", { name: "Bestätigen", exact: true }).first().click()
  await incomeCard.getByPlaceholder("Betrag").last().fill("900")
  await incomeCard.getByLabel("Wirksamkeitsdatum").fill("2026-08-01")
  await incomeCard.getByRole("button", { name: "Aktion bestätigen" }).click()

  await expect(incomeCard.locator("p", { hasText: "bestätigt" }).first()).toBeVisible()
