...
public static partial class BudgetEndpoints
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        (BudgetPeriod? period, List<BudgetCategory> _, List<BudgetAccount> _) = await new BudgetService(database, timeProvider)
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(date) && !DateOnly.TryParseExact(
                date, "yyyy-MM-dd", null, System.Globalization.DateTimeStyles.None, out selectedDate))
            return HttpResults.Problem(400, "Invalid date", "Date must use YYYY-MM-DD");
        return Results.Ok(await new BudgetService(database, timeProvider).SummaryAsync(user.Id, selectedDate, cancellationToken));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        BudgetService service = new BudgetService(database, timeProvider);
        (BudgetPeriod? period, List<BudgetCategory> _, List<BudgetAccount> _) = await service.EnsureDefaultsAsync(user.Id, DateOnly.FromDateTime(timeProvider.GetUtcNow().UtcDateTime), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        CurrentUser? user = await identity.CurrentUserAsync(context, cancellationToken);
        if (user is null) return Unauthorized();
        (string Name, string Color, string Icon, string Behavior, IResult? Error) validation = ValidateCategory(request);
        ...
        CancellationToken cancellationToken)
    {
        ...
        (string Name, string Color, string Icon, string Behavior, IResult? Error) validation = ValidateCategory(request);
        if (validation.Error is not null) return validation.Error;
        BudgetCategory? category = await database.Categories.SingleOrDefaultAsync(
        ...
        CancellationToken cancellationToken)
    {
        CurrentUser? user = await identity.CurrentUserAsync(context, cancellationToken);
        if (user is null) return Unauthorized();
        (PlannedExpense? Value, IResult? Error) parsed = await ParsePlannedExpense(request, user.Id, database, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        CurrentUser? user = await identity.CurrentUserAsync(context, cancellationToken);
        if (user is null) return Unauthorized();
        PlannedExpense? existing = await database.PlannedExpenses.SingleOrDefaultAsync(
        ...
        (PlannedExpense? Value, IResult? Error) parsed = await ParsePlannedExpense(request, user.Id, database, cancellationToken);
        if (parsed.Error is not null) return parsed.Error;
        PlannedExpense value = parsed.Value!;
        ...
        CancellationToken cancellationToken)
    {
        ...
        BudgetService service = new BudgetService(database, timeProvider);
        (BudgetPeriod? period, List<BudgetCategory> _, List<BudgetAccount> _) = await service.EnsureDefaultsAsync(user.Id, DateOnly.FromDateTime(timeProvider.GetUtcNow().UtcDateTime), cancellationToken);
        List<PlannedExpense> plans = await database.PlannedExpenses.Where(x => x.OwnerUserId == user.Id && x.Active)
            .OrderBy(x => x.DueDay).ThenBy(x => x.Name).ToListAsync(cancellationToken);
        HashSet<Guid> existing = (await database.PlannedExpenseApplications.Where(x => x.OwnerUserId == user.Id && x.PeriodId == period.Id)
            .Select(x => x.PlannedExpenseId).ToListAsync(cancellationToken)).ToHashSet();
        int applied = 0; int skipped = 0;
        foreach (PlannedExpense? plan in plans)
        {
            DateOnly? occurredOn = BudgetService.OccurrenceDate(period, plan);
            if (occurredOn is null || existing.Contains(plan.Id)) { skipped++; continue; }
            BudgetTransaction entry = new BudgetTransaction
            {
        ...
        CancellationToken cancellationToken)
    {
        ...
        BudgetAccount? account = await database.Accounts.SingleOrDefaultAsync(
        ...
        BudgetService service = new BudgetService(database, timeProvider);
        (BudgetPeriod? period, List<BudgetCategory> _, List<BudgetAccount> _) = await service.EnsureDefaultsAsync(user.Id, occurredOn, cancellationToken);
        BudgetTransaction entry = new BudgetTransaction
        {
    ...
    private static (string Name, string Color, string Icon, string Behavior, IResult? Error) ValidateCategory(CategoryRequest request)
    {
        ...
        string color = string.IsNullOrWhiteSpace(request.Color) ? "#64748b" : request.Color.Trim();
        if (!HexColor().IsMatch(color)) return ("", "", "", "", HttpResults.Problem(422, "Validation failed", "category color must be a #RRGGBB value"));
        string icon = string.IsNullOrWhiteSpace(request.Icon) ? "tag" : request.Icon.Trim().ToLowerInvariant();
        if (!IconName().IsMatch(icon)) return ("", "", "", "", HttpResults.Problem(422, "Validation failed", "category icon is invalid"));
        string behavior = string.IsNullOrWhiteSpace(request.Behavior) ? BudgetValues.IncludeInLimit : request.Behavior.Trim();
        ...
        PlannedExpenseRequest request, Guid ownerId, BudgetDbContext database, CancellationToken cancellationToken)
    {
        ...
        int dueDay = request.DueDay == 0 ? 1 : request.DueDay;
        ...
        string kind = string.IsNullOrWhiteSpace(request.Kind) ? BudgetValues.FixedCost : request.Kind;
        ...
        string cadence = string.IsNullOrWhiteSpace(request.Cadence) ? BudgetValues.Monthly : request.Cadence;
        ...
        int? dueMonth = cadence == BudgetValues.Yearly ? request.DueMonth : null;
        ...
        bool accountExists = await database.Accounts.AnyAsync(x => x.Id == request.AccountId && x.OwnerUserId == ownerId, cancellationToken);
