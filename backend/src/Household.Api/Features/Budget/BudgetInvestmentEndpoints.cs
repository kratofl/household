...
public static class BudgetInvestmentEndpoints
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (key is not null)
        {
            BudgetInvestmentEvent? existing = await database.InvestmentEvents.AsNoTracking()
        ...
        if (kind == BudgetValues.Withdrawal)
        {
            ...
            InvestmentProjection projection = await new BudgetInvestmentProjector(database).LoadAsync(
        ...
        BudgetInvestmentEvent item = new BudgetInvestmentEvent
        {
            OwnerUserId = user.Id,
            PeriodId = period.Id,
            IdempotencyKey = key,
        ...
        catch (DbUpdateException) when (key is not null)
        {
            ...
            BudgetInvestmentEvent? winner = await database.InvestmentEvents.AsNoTracking()
                .SingleOrDefaultAsync(x => x.OwnerUserId == user.Id && x.IdempotencyKey == key, cancellationToken);
