...
public sealed class UpdatesClient(HttpClient httpClient)
{
    ...
    {
        string repository = HouseholdConfiguration.String("HOUSEHOLD_UPDATES_GITHUB_REPOSITORY", "kratofl/household").Trim('/');
        using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, $"https://api.github.com/repos/{repository}/releases?per_page=20");
    ...
    {
        string baseUrl = HouseholdConfiguration.String("HOUSEHOLD_UPDATES_UPDATER_URL").TrimEnd('/');
        ...
        string token = HouseholdConfiguration.String("HOUSEHOLD_UPDATES_UPDATER_TOKEN");
